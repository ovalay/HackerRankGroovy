...
/**
 * Created by Tim Sloan on 06/11/15.
 */
...
System.in.withReader { br ->
    ...
    testCases.times {
        def entry = br.readLine().tokenize()*.toInteger()
        def totalTime = entry.sum()

        if (orders.get(totalTime))
            orders[totalTime] << it+1
        else
            orders [totalTime] = [it+1]
    }

    orders = orders.sort()
    println  orders.collect {mapEntry ->
        if (mapEntry.value.size > 1) {
            mapEntry.value.sort().join(' ')
        }
        else {
            mapEntry.value.join(' ')
        }
